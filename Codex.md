...
- 2026-05-23 CT AI/cost decision: testing should use `amazon.nova-micro-v1:0` again as the prepared Bedrock default, keep `BEDROCK_USE_CROSS_REGION_INFERENCE=false`, set `BEDROCK_MONTHLY_TOKEN_BUDGET=15000000`, and keep `BEDROCK_MONTHLY_BUDGET_USD=5` as the AWS Bedrock alert cap. Cost Explorer API refreshes must be counted in the dashboard cost model as `USD 0.01` per successful refresh for the selected range. Production Lambda invocation totals around `46,512` for 2026-05-01 to 2026-05-23 came from CloudWatch, mostly `moyra-production-api`; API Gateway route breakdown showed the dominant traffic as `GET /api/v2/main` (~19.3k) plus `GET /api/v2/publications`, `GET /api/v2/articles`, and `GET /api/v2/services` (~9k each), consistent with public frontend/homepage data loads rather than admin-only traffic.
